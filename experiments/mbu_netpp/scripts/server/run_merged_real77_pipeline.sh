...
REAL26_RAW="${REAL26_RAW:-$SERVER_ASSETS_ROOT/training_sources/analysis_real26}"
REAL16_RAW="${REAL16_RAW:-$SERVER_ASSETS_ROOT/training_sources/refined_real16}"
NASA_PREPARED="${NASA_PREPARED:-$SERVER_ASSETS_ROOT/training_sources/prepared_nasa_super_secondary_train}"
FINAL_INFER_INPUT="${FINAL_INFER_INPUT:-$SERVER_ASSETS_ROOT/final_infer_inputs/full_png_cropped_xlsx_images}"

PREPARED_REAL26="${PREPARED_REAL26:-$REPO_ROOT/experiments/mbu_netpp/workdir/prepared_analysis_real26}"
PREPARED_REAL16="${PREPARED_REAL16:-$REPO_ROOT/experiments/mbu_netpp/workdir/prepared_refined_real16}"
MERGED_PREPARED="${MERGED_PREPARED:-$REPO_ROOT/experiments/mbu_netpp/workdir/prepared_merged_real77}"
TRAIN_OUTPUT="${TRAIN_OUTPUT:-$REPO_ROOT/experiments/mbu_netpp/outputs/merged_real77_supervised}"
...
DELIVERY_NAME="${DELIVERY_NAME:-merged_real77_final_infer_100}"
DELIVERY_DIR="${DELIVERY_DIR:-$DELIVERY_ROOT/$DELIVERY_NAME}"

BASE_CONFIG_PATH="${BASE_CONFIG_PATH:-$REPO_ROOT/experiments/mbu_netpp/configs/merged_real77_supervised_server_full.yaml}"
CONFIG_PATH="${CONFIG_PATH:-$REPO_ROOT/experiments/mbu_netpp/workdir/merged_real77_server_runtime.yaml}"
...
for required_dir in "${REAL26_RAW}" "${REAL16_RAW}" "${NASA_PREPARED}" "${FINAL_INFER_INPUT}"; do
...
if [[ "${CLEAN_RUN}" == "1" ]]; then
  rm -rf "${PREPARED_REAL26}" "${PREPARED_REAL16}" "${MERGED_PREPARED}" "${TRAIN_OUTPUT}" "${DELIVERY_DIR}"
  rm -f "${DELIVERY_ROOT}/${DELIVERY_NAME}.tar.gz"
fi

python -m experiments.mbu_netpp.preparation \
  --images-dir "${REAL26_RAW}" \
  --annotations-dir "${REAL26_RAW}" \
  --output-root "${PREPARED_REAL26}" \
  --no-auto-crop
...
python -m experiments.mbu_netpp.prepare_merged_supervised \
  --prepared-root "${PREPARED_REAL26}" \
  --source-alias real26 \
  --prepared-root "${PREPARED_REAL16}" \
  --source-alias real16 \
  --prepared-root "${NASA_PREPARED}" \
  --source-alias nasa \
...
summary_path = Path("experiments/mbu_netpp/outputs/merged_real77_supervised/crossval_summary.json")
...
cp -r "${FINAL_INFER_OUTPUT}" "${DELIVERY_DIR}"
