...
class VMM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # 对原代码进行了修改， def forward(self, x: torch.Tensor, mair_ids, **kwargs)，原代码输入要求为x，mair_ids
        # -------------------------------------------
        xs_scan_ids, xs_inverse_ids = mair_ids_generate(inp_shape=(1, 1, H, W), scan_len=4)  # [B,H,W,C]

        xs_shift_scan_ids, xs_shift_inverse_ids = mair_shift_ids_generate(inp_shape=(1, 1, H, W),
                                                                          scan_len=4,
                                                                          shift_len=4 // 2)  # [B,H,W,C]
        if torch.cuda.is_available():
            xs_scan_ids, xs_inverse_ids, xs_shift_scan_ids, xs_shift_inverse_ids = \
                xs_scan_ids.cuda(), xs_inverse_ids.cuda(), xs_shift_scan_ids.cuda(), xs_shift_inverse_ids.cuda()
        # ----------------------------------------------
        xz = self.in_proj(x)
        ...
        y = self.forward_core(x, (xs_scan_ids, xs_inverse_ids, xs_shift_scan_ids, xs_shift_inverse_ids))
...
# 输入 B H W C,  输出B H W C
if __name__ == '__main__':
    block = VMM(d_model=64).cuda()
