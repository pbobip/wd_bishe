...
import numpy as np
# 论文：Efficient Visual State Space Model for Image Deblurring
...
if __name__ == '__main__':
    # 构造一个随机输入图像，要求批量大小为1（因 EVS.grids 中有 assert b == 1）
    #如果batch size不想为1，直接把246行代码assert b ==1 给注释掉即可
    input = torch.randn(1, 3, 64, 64).cuda
    # 实例化 EVSSM 模型（图像去模糊网络）
    model = EVSSM().cuda
    # 前向传播获得输出图像
    output = model(input)
