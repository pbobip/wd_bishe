...
# 论文：HVI: ANewColor Space for Low-light Image Enhancement
...
if __name__ == '__main__':
    # B: batch_size, C: 通道数, H, W: 图像尺寸
    B, C, H, W = 2, 64, 32, 32
    num_heads = 8
    input_x = torch.randn(B, C, H, W)
    input_y = torch.randn(B, C, H, W)
    hv_lca = HV_LCA(dim=C, num_heads=num_heads, bias=True)
    out_hv_lca = hv_lca(input_x, input_y)
    print("HV_LCA:", out_hv_lca.shape)
    i_lca = I_LCA(dim=C, num_heads=num_heads, bias=True)
    out_i_lca = i_lca(input_x, input_y)
    print("I_LCA:", out_i_lca.shape)
